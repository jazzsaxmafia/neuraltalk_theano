...
def preProBuildWordVocab(sentence_iterator, word_count_threshold):
  ...
  '''
  모든 sentence의 단어들을 카운트, word_count_threshold 넘는 경우만 남김
  word->idx, idx->word 사전 return
  '''
  ...
  # for example on flickr30k, doing this brings down initial perplexity from ~2500 to ~170.
  word_counts['.'] = nsents
...
def RNNGenCost(batch, model, params, misc):
  ...
  for i,pair in enumerate(batch):
    ...
    Y = Ys[i] # 각 단어에 대한 activation
    maxes = np.amax(Y, axis=1, keepdims=True)
    e = np.exp(Y - maxes) # for numerical stability shift into good numerical range
    P = e / np.sum(e, axis=1, keepdims=True) # Softmax Probability
    ''' Numerically Stable way of calculating SoftMax??  '''
    ...
    '''
    delta (dY/dz) : y - t
    gtix: 문장의 각 정답 단어 index. 그 부분만 1, 나머지는 0
    정답 단어란: 바로 다음에 나오는 단어.
    '''
    dYs.append(P)
...
def main(params):
  ...
  '''
  사실상 중요한 정보들은 misc 안에 모두 저장되어 있다
  예를 들어 word->index, index->word 사전이나 각종 weight값 등등
  '''
...
if __name__ == "__main__":
  ...
  parser.add_argument('-d', '--dataset', dest='dataset', default='flickr30k', help='dataset: flickr30k/flickr30k')
