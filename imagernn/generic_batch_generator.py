...
'''
driver.py에서 decodeGenerator를 호출할 때는 이 GenericBatchGenerator가 호출되고,
이 GenericBatchGenerator에서 decodeGenerator를 호출한 경우에는 LSTM/RNN Generataor가 호출됟
'''
...
class GenericBatchGenerator:
  ...
  def forward(batch, model, params, misc, predict_mode = False):
    ...
    for i,x in enumerate(batch):
      ...
      ''' [0]: #START# '''
      Xs = np.row_stack( [Ws[j, :] for j in ix] ) # word -> vector Xs: 단어 수 x 각 단어의 vector
      Xi = Xe[i,:] # Xi: 이미지 -> image encoding
