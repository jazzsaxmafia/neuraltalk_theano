...
'''
self.dataset :
    self.dataset['dataset']: 데이터셋 종류 ( flickr8k, flickr30k, coco 중 1 )
    self.dataset['images'] : 각 이미지에 대한 정보
        - filename
        - imgid
        - sentences
...
self.features : 각 이미지의 4096 vgg features
'''
...
class BasicDataProvider:
  ...
  def sampleImageSentencePair(self, split = 'train'):
    ...
    db에서 image-sentence pair 하나 sapling
    """
  ...
  def iterImageSentencePairBatch(self, split = 'train', max_images = -1, max_batch_size = 100):
    """
    max_batch_size 크기의 batch return
    """
    batch = []
    for i,img in enumerate(self.split[split]):
      if max_images >= 0 and i >= max_images: break
